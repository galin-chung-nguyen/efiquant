...
const SignInForm = () => {
  ...
  const handleSubmitSignInForm = async (e: any) => {
    e.preventDefault();
    ...
    try {
      const { data: { login: { user, access_token } } } = await makeGraphqlMutation(gql`
  ...
  return (<form className='py-12'>
    <h2 className='text-3xl font-bold text-infinaPrimary mb-5'>Sign in</h2>
    ...
    <p className='text-gray-500 mb-3'>or use your Infina account</p>
    <div className='flex flex-col items-center'>
      ...
      <button className='border-2 border-infinaPrimary rounded-full px-12 py-2 inline-block font-semibold hover:bg-infinaPrimary hover:text-white' onClick={handleSubmitSignInForm}>Next</button>
    </div>
  </form>);
}
      ...
      <main className="flex w-full flex-1 flex-col items-center justify-center px-20 text-center">
        <div className="bg-white rounded-2xl bg-red shadow-2xl flex w-3/4 max-w-4xl">
          ...
          <div className='w-2/5 bg-infinaPrimary flex flex-col justify-center text-white rounded-tr-2xl rounded-br-2xl py-36 px-12'>
            ...
            <p className='mt-2'>
              <Link href='/sign-up'>
                <a className='text-white hover:underline'>I am new to this platform.<br />Create Link new account.</a>
              </Link>
            </p>
